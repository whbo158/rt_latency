/*
 * Author: WangHongbo(whbo158@qq.com)
 */
...
static int print_benchmark(void)
{
	stat_val_t *ps = &swake_stat;

	printf("cnt,%d,sec,%d,time,%d,%d,%d,%.2f,index,%d,%d\n", \
		ps->cnt, scur_sec, ps->min, ps->val, ps->max, ps->mean, ps->min_idx, ps->max_idx);
...
static void print_usage(char *pname)
{
	...
	printf("\t-p: print benchmark every second\n");
	printf("\t-c: cycle_time, 125000ns in default\n");
	printf("\t-t: total test seconds, 0 in default\n");
	printf("\n");
}
